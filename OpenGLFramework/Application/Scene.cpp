...
namespace Application
{
	...
	Scene::Scene(std::unique_ptr<IWindow> _window)
	{
		window = std::move(_window);

		std::shared_ptr<IO::TextureLoader> textureLoader = std::make_shared<IO::TextureLoader>(IO::TextureLoader());
		textureLoader->Initialize();

		std::shared_ptr<IO::MeshLoader> meshLoader = std::make_shared<IO::MeshLoader>(IO::MeshLoader());
		meshLoader->Initialize(textureLoader);

		basicShader = std::make_shared<Rendering::BasicShader>(Rendering::BasicShader());
		advancedShader = std::make_shared<Rendering::AdvancedShader>(Rendering::AdvancedShader());
		customShader = std::make_shared<CustomShader>(Application::CustomShader());
		colorShader = std::make_shared<Rendering::ColorShader>(Rendering::ColorShader());
	...
	void Scene::input()
	{
		...
		std::shared_ptr<Input::InputHandler> inputHandler = window->GetInputHandler();

		camera->Update();

		// Rotation
		camera->Rotate(inputHandler->GetCursorDelta().x / 5, Math::Vector3(1.0f, 0.0f, 0.0f));
		camera->Rotate(inputHandler->GetCursorDelta().y / 5, Math::Vector3(0.0f, 1.0f, 0.0f));
